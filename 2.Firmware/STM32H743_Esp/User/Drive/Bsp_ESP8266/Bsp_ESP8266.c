 ...
 * @LastEditTime: 2022-08-16 19:04:42
 */
...
uint8_t Bsp_ESP8266_Auto_LinkWifi(uint8_t Enable)
{
  ...
  if (Bsp_ESP8266_WaitResponse("OK", 1000) == 1)
  {
    return 1;
  }
...
uint8_t Bsp_ESP8266_CreateTCPServer(uint16_t _TcpPort)
{
  ...
  Bsp_ESP8266_SendAT(cmd_buf);
  if (Bsp_ESP8266_WaitResponse("OK", 2000) == 1)
  {
    return 1;
  }
...
uint8_t Bsp_ESP8266_SendTcpUdp(uint8_t _id, uint8_t *_databuf, uint16_t _len)
{
  ...
  if (Bsp_ESP8266_WaitResponse(">", 1000) == 1)
  {
    return 1;
  }
  ...
  if (Bsp_ESP8266_WaitResponse("SEND OK", 8000) == 1)
  {
    return 1;
  }
...
void Bsp_ESP8266_CloseTcpUdp(uint8_t _id)
{
  ...
  Bsp_ESP8266_WaitResponse("OK", 200);
}
