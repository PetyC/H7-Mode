...
/**
* @brief check CRC for data 判断接收的数据CRC是否正确
...
{
  uint16_t Receive_CRC = 0, calculation = 0;
  ...
  Receive_CRC = crc16_modbus(modbusdata, length-4);

  calculation = modbusdata[length-4];

  calculation <<= 8;

  calculation += modbusdata[length-3];
